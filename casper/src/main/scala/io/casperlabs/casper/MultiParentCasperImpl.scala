...
class MultiParentCasperImpl[F[_]: Bracket[?[_], Throwable]: Log: Time: FinalityDetector: BlockStorage: DagStorage: ExecutionEngineService: LastFinalizedBlockHashContainer: DeployStorageWriter: DeployStorageReader: Validation](
  ...
  private def remainingDeploys(
    ...
    for {
      ...
      pendingDeploys  <- DeployStorageReader[F].readPending
  ...
  private def requeueOrphanedDeploys(
    ...
    for {
      ...
      orphanedDeploys <- findOrphanedDeploys(dag, parents)
      _               <- DeployStorageWriter[F].markAsPending(orphanedDeploys) whenA orphanedDeploys.nonEmpty
  ...
  private def findOrphanedDeploys(
    ...
    for {
      processedDeploys <- DeployStorageReader[F].readProcessed
